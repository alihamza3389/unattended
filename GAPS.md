...
Some nights that has failed. When it does there is a hole in `corpus/nights`
where a night should be, and anyone counting can see it. This file says what
happened on each one, because a record you can check is worth nothing if the
missing parts go unexplained, and because whoever notices a gap is exactly the
person owed an answer.
